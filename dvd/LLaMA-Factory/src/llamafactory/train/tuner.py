...
# Lazy imports for non-SFT stages (avoid trl/torch compatibility issues)
_run_dpo = None
def _get_run_dpo():
    global _run_dpo
    if _run_dpo is None:
        from .dpo import run_dpo as _dpo
        _run_dpo = _dpo
    return _run_dpo
...
def _get_run_kto():
    global _run_kto
    if _run_kto is None:
        from .kto import run_kto as _kto
        _run_kto = _kto
    return _run_kto
...
def _get_run_ppo():
    global _run_ppo
    if _run_ppo is None:
        from .ppo import run_ppo as _ppo
        _run_ppo = _ppo
    return _run_ppo
...
def _training_function(config: dict[str, Any]) -> None:
    ...
    if finetuning_args.stage == "pt":
        _get_run_pt()(model_args, data_args, training_args, finetuning_args, callbacks)
    elif finetuning_args.stage == "sft":
        run_sft(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)
    elif finetuning_args.stage == "rm":
        _get_run_rm()(model_args, data_args, training_args, finetuning_args, callbacks)
    elif finetuning_args.stage == "ppo":
        _get_run_ppo()(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)
    elif finetuning_args.stage == "dpo":
        _get_run_dpo()(model_args, data_args, training_args, finetuning_args, callbacks)
    elif finetuning_args.stage == "kto":
        _get_run_kto()(model_args, data_args, training_args, finetuning_args, callbacks)
    else:
