...
class DVDEMACallback(TrainerCallback):
    r"""EMA update for DVD teacher ViT (after optimizer.step, GA-safe)."""

    def on_step_end(self, args, state, control, **kwargs):
        model = kwargs["model"]
        unwrapped = model.module if hasattr(model, "module") else model
        ema = getattr(unwrapped.config, "ema_decay", 1.0)
        if ema >= 1.0 or not hasattr(unwrapped, "visual_teacher"):
            return
        with torch.no_grad():
            for student, teacher in zip(
                unwrapped.visual.parameters(),
                unwrapped.visual_teacher.parameters(),
            ):
                teacher.data.mul_(ema).add_(student.data, alpha=1.0 - ema)
