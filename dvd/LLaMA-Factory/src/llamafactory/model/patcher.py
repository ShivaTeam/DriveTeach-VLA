...
def patch_config(
    ...
    if model_args.enable_dvd:
        ...
        config.vit_loss_weight = model_args.dvd_tgp_weight
        config.ema_decay = model_args.dvd_ema
    else:
        config.ema_decay = 1.0
...
def patch_model(
    ...
    # DVD: copy student ViT weights to teacher ViT
    if model_args.enable_dvd and hasattr(model, "visual_teacher"):
        with torch.no_grad():
