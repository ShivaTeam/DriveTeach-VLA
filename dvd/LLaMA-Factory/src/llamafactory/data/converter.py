...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.dataset_attr.enable_dvd and example.get("bbox_image"):
            bbox = self._find_medias(example["bbox_image"])
            images = (images or []) + (bbox if isinstance(bbox, list) else [bbox])
        output = {
            "_prompt": prompt,
            "_response": response,
