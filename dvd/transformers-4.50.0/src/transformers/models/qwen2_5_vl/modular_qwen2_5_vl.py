...
class Qwen2_5_VLForConditionalGeneration(Qwen2VLForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self.visual_teacher = Qwen2_5_VisionTransformerPretrainedModel._from_config(config.vision_config)
        
        for param in self.visual_teacher.parameters():
            param.requires_grad = False

        self.vit_loss_fct = SmoothL1Loss()
        self.vit_loss_weight = config.vit_loss_weight if hasattr(config, "vit_loss_weight") else 0.5
    ...
    def forward(
        ...
        if image_grid_thw.shape[0] >= 4:
            lim = pixel_values.shape[0] / image_grid_thw.shape[0]
            image_grid_thw_teacher = image_grid_thw[2:4]
            pixel_values_teacher = pixel_values[lim:2*lim]
            image_grid_thw = image_grid_thw[0:2]
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                if not image_grid_thw_teacher is None:
                    image_embeds_teacher = self.visual_teacher(pixel_values_teacher, grid_thw=image_grid_thw_teacher)
                ...
                if not image_grid_thw_teacher is None:
                    image_embeds_teacher.to(inputs_embeds.device, inputs_embeds.dtype)
        ...
        if labels is not None:
            ...
            if not image_embeds_teacher is None:
                vit_loss = self.vit_loss_fct(image_embeds, image_embeds_teacher)
                loss = loss + vit_loss
