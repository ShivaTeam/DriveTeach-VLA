...
def calculate_regional_loss(t1: torch.Tensor, t2: torch.Tensor, h: int, w: int, hs: int, ws: int) -> torch.Tensor:
    """Block-wise SmoothL1 distillation loss used in DVD (Driving-aware Vision Distillation).

    Partitions the ViT feature map into hs * ws blocks, computes the mean feature
    within each block, and returns the SmoothL1 loss between the two inputs.

    Steps:
    1. Reshape t1, t2 from (2*h*w, C) to (2, h, w, C)
    2. Average over dim 0 → p1, p2 of shape (h, w, C)
    3. Partition into hs x ws blocks: reshape to (hs, h/hs, ws, w/ws, C),
       then mean-pool within each block → (hs, ws, C)
    4. Flatten to (hs*ws, C) and compute SmoothL1 loss

    Args:
        t1, t2: ViT patch embeddings, shape (2*h*w, C) where C = 2048
        h, w: spatial resolution of the ViT feature map
        hs, ws: number of blocks along height and width (must divide h and w)

    Returns:
        Scalar SmoothL1 loss.
    """
    if h % hs != 0:
        raise ValueError(f"h={h} must be divisible by hs={hs}.")
    if w % ws != 0:
        raise ValueError(f"w={w} must be divisible by ws={ws}.")

    C = 2048
    expected = (2 * h * w, C)
    if t1.shape != expected or t2.shape != expected:
        raise ValueError(
            f"Expected shape {expected}, got t1={t1.shape}, t2={t2.shape}"
        )

    # (1) Reshape to separate spatial dims
    s1 = t1.view(2, h, w, C)
    s2 = t2.view(2, h, w, C)

    # (2) Pool over the leading 2-channel group: (2, h, w, C) → (h, w, C)
    p1 = torch.mean(s1, dim=0)
    p2 = torch.mean(s2, dim=0)

    # (3) Block-wise mean: (h, w, C) → (hs, h/hs, ws, w/ws, C) → (hs, ws, C)
    p1_blocks = p1.view(hs, h // hs, ws, w // ws, C)
    p2_blocks = p2.view(hs, h // hs, ws, w // ws, C)
    m1 = p1_blocks.mean(dim=(1, 3))
    m2 = p2_blocks.mean(dim=(1, 3))

    # (4) Flatten and compute SmoothL1: (hs*ws, C)
    m1 = m1.view(hs * ws, C)
    m2 = m2.view(hs * ws, C)

    return F.smooth_l1_loss(m1, m2)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.h_size = config.h_size if hasattr(config, "h_size") else 16
        self.w_size = config.w_size if hasattr(config, "w_size") else 28
