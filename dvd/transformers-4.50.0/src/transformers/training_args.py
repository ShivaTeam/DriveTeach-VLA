...
class TrainingArguments:
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if "ACCELERATE_use_configured_state" in os.environ and os.environ["ACCELERATE_use_configured_state"] == "False":
            accelerator_state_kwargs["use_configured_state"] = False
        ...
        if self.distributed_state is None:
            ...
            self.local_rank = 0
            self.distributed_state = DistributedType.NO
            self.distributed_state.distributed_type = DistributedType.NO
            self.distributed_state.num_processes = 1
            self.distributed_state.process_index = 0
            self.distributed_state.local_process_index = 0
        elif not is_sagemaker_mp_enabled():
